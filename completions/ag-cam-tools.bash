...
_ag_cam_tools() {
    ...
    case "${prev}" in
        ...
        --calibration-slot)
            ...
            ;;
        --stereo-backend)
            COMPREPLY=( $(compgen -W "sgbm onnx igev rt-igev foundation" -- "${cur}") )
            return 0
            ;;
        --model-path)
    ...
    case "${subcmd}" in
        ...
        list)
            ...
            ;;
        capture)
            COMPREPLY=( $(compgen -W "-s --serial -a --address -i --interface -o --output -e --encode -x --exposure -g --gain -A --auto-expose -b --binning -p --packet-size --burst --calibration-local --calibration-slot -t --tag-size -v --verbose -h --help" -- "${cur}") )
            ;;
        stream)

                  ...
                  AgCameraConfig *out)
{
     ...
     *
     * 2x2 Average binning (hardware or software) mixes R/G/G/B samples
     * into a single value, destroying the CFA mosaic.  Default to
     * treating binned data as grayscale-like unless the camera's ISP
     * reports a valid Bayer pattern after binning. */
    if (binning <= 1) {
        out->data_is_bayer = TRUE;
    } else {
        out->data_is_bayer = FALSE;

        /* If the camera preserves a valid Bayer arrangement even after
         * binning, IspBayerPattern will report something other than
         * "None".  Use that as positive evidence to re-enable debayer. */
        GError *isp_err = NULL;
        const char *isp_pat = arv_device_get_string_feature_value (
            device, "IspBayerPattern", &isp_err);
        if (!isp_err && isp_pat &&
            strcmp (isp_pat, "None") != 0 &&
            strcmp (isp_pat, "NONE") != 0) {
            out->data_is_bayer = TRUE;
            printf ("  IspBayerPattern = %s "
                    "(binned data preserves Bayer CFA)\n", isp_pat);
        } else {
            printf ("  IspBayerPattern = %s "
                    "(binned data treated as grayscale)\n",
                    (isp_pat ? isp_pat : "(unavailable)"));
        }
        g_clear_error (&isp_err);
    }
